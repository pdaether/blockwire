            ...
            <aside x-cloak x-show="sidebarVisible" class="shrink-0 relative bg-white overflow-hidden border-l border-gray-200" :style="panelStyle()">
                ...
                <div wire:key="active-block-panel-{{ $activeBlockIndex }}" wire:transition class="border-b border-gray-200 mb-4">
                    <div class="border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                        ...
                        @php
                            $activeBlockShow = !isset($activeBlocks[$activeBlockIndex]['show']) || $activeBlocks[$activeBlockIndex]['show'] !== false;
                        @endphp
                        <div x-data="{ actionMenuOpen: false }" class="relative flex items-center shrink-0 border-l border-gray-200 pl-2">
                            <button
                                wire:click="toggleBlockVisibility"
                                class="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-xs font-medium transition-colors {{ $activeBlockShow ? 'border-green-200 bg-green-50 text-green-600 hover:bg-green-100' : 'border-red-200 bg-red-50 text-red-600 hover:bg-red-100' }}"
                                aria-label="{{ $activeBlockShow ? 'Hide block' : 'Show block' }}"
                                title="{{ $activeBlockShow ? 'Visible on frontend' : 'Hidden on frontend' }}">
                                @if($activeBlockShow)
                            ...
                            <button
                                x-on:click.prevent="actionMenuOpen = !actionMenuOpen"
                                aria-label="Block actions"
                                title="Block actions"
                                class="ml-2 shrink-0 p-3 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-md">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM12.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM18.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                                </svg>
                            </button>
                            ...
                            <div
                                ...
                                class="absolute right-0 top-full z-40 mt-2 w-40 overflow-hidden rounded-md border border-gray-200 bg-white shadow-xl">
                                <button x-on:click.prevent="cloneActiveBlock(); actionMenuOpen = false" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 01-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 011.5.124m7.5 10.376h3.375c.621 0 1.125-.504 1.125-1.125V11.25c0-4.46-3.243-8.161-7.5-8.876a9.06 9.06 0 00-1.5-.124H9.375c-.621 0-1.125.504-1.125 1.125v3.5m7.5 10.375H9.375a1.125 1.125 0 01-1.125-1.125v-9.25m12 6.625v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5a3.375 3.375 0 00-3.375-3.375H9.75" />
                                    </svg>
                                    <span>{{ __('Copy') }}</span>
                                </button>
                                <button x-on:click.prevent="deleteActiveBlock(); actionMenuOpen = false" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
