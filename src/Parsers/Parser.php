...
abstract class Parser
{
    ...
    protected function shouldRenderBlock(array $block): bool
    {
        if ($this->context === 'editor') {
            return true;
        }

        return $this->normalizeShowFlag($block['show'] ?? true);
    }

    public function prepareBlockForEditor(array $args)
    {
        $activeBorderColor = config('blockwire.brand.colors.active_border', 'blue-500');
        $show = $this->normalizeShowFlag($args['show'] ?? true);
        ...
        $args = collect([
            ...
            'id' => $args['id'],
            'before' => '<div drag-item draggable="true" class="[&_*]:pointer-events-none relative hover:opacity-75 hover:cursor-pointer before:opacity-0 hover:before:opacity-100 before:absolute before:top-0 before:left-0 before:w-full before:h-full before:border-2 before:border-gray-400 after:opacity-0 after:absolute after:left-0 after:w-full [&.active]:before:opacity-100" style="--active-border-color: '.$borderColor.'" data-block="'.$args['id'].'" data-show="'.($show ? '1' : '0').'">',
    ...
    {
        return '<div drop-placeholder class="h-full min-h-[300px] text-gray-400 text-lg flex flex-col gap-3 items-center justify-center border-2 border-dashed border-gray-300 rounded-lg m-4 transition-all duration-200"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg><p>'.__('Drop your block here...').'</p></div>';
    }
