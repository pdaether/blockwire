...
abstract class Parser
{
    ...
    public function prepareBlockForEditor(array $args)
    {
        ...
        $args = collect([
            ...
            'id' => $args['id'],
            'before' => '<div drag-item draggable="true" class="[&_*]:pointer-events-none relative hover:opacity-75 hover:cursor-pointer before:opacity-0 hover:before:opacity-100 before:absolute before:top-0 before:left-0 before:w-full before:h-full before:border-2 before:border-gray-400 after:opacity-0 after:absolute after:bg-gray-400 after:left-0 after:w-full [&.active]:before:opacity-100" style="--active-border-color: '.$borderColor.'" data-block="'.$args['id'].'" data-show="'.($show ? '1' : '0').'">',
